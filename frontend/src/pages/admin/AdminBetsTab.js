    ...
    <Card className="bg-gray-50 border-gray-200">
      <CardHeader>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <CardTitle className="text-gray-900 font-['Unbounded'] flex items-center gap-2">
          ...
          <div className="flex gap-2">
            <Select value={betDistGame} onValueChange={setBetDistGame}>
              <SelectTrigger className="w-40 bg-white border-gray-200 text-gray-900"><SelectValue placeholder="सभी गेम्स" /></SelectTrigger>
              <SelectContent className="bg-gray-50 border-gray-200">
                <SelectItem value="all" className="text-gray-900 hover:bg-gray-100">सभी गेम्स</SelectItem>
                {games.map((game) => (
            ...
            <Popover>
              ...
              <PopoverContent className="w-auto p-0 bg-gray-50 border-gray-200">
                <Calendar mode="single" selected={betDistDate} onSelect={(date) => date && setBetDistDate(date)} className="bg-gray-50" />
              </PopoverContent>
            </Popover>
          </div>
        </div>
      ...
      <CardContent>
        {loadingBetDist ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 text-lg">इस तारीख पर कोई पेंडिंग बेट नहीं</p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="p-4 bg-white rounded-lg text-center">
                <p className="text-gray-500 text-sm">कुल बेट्स</p>
                <p className="text-2xl font-bold text-gray-900">{betDistribution.summary.total_bets}</p>
              </div>
            ...
            {Object.entries(betDistribution.distribution).map(([gameId, gameData]) => (
              <div key={gameId} className="mb-6">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-lg font-bold text-gray-900">{gameData.game_name}</h3>
                  <span className="text-emerald-400 text-sm">राशि: ₹{gameData.total_amount}</span>
                </div>

                {Object.keys(gameData.jodi).length > 0 && (
                  <div className="mb-4">
                    <p className="text-gray-500 text-sm mb-2">जोड़ी बेट्स:</p>
                    <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2">
                      {Object.entries(gameData.jodi).map(([number, data]) => (
                        <div key={number} className="p-3 bg-white rounded-lg border border-gray-200 hover:border-[#D4AF37]/50 transition-all">
                          <div className="text-center">
                            <span className="text-2xl font-bold text-[#D4AF37]">{number}</span>
                            <div className="mt-1">
                              <p className="text-gray-900 font-semibold">₹{data.amount}</p>
                              <p className="text-gray-500 text-xs">{data.count} बेट</p>
                            </div>
                          </div>
                        </div>
                      ))}
                  ...
                  <div className="mt-4">
                    ...
                    <div className="grid grid-cols-5 sm:grid-cols-10 gap-2">
                      {Object.entries(gameData.haruf_andar).map(([number, data]) => (
                        <div key={number} className="p-2 bg-white rounded-lg border border-blue-500/20 text-center">
                          <span className="text-xl font-bold text-blue-400">{number}</span>
                          <p className="text-gray-900 text-sm">₹{data.amount}</p>
                          <p className="text-gray-500 text-xs">{data.count} बेट</p>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="grid grid-cols-5 sm:grid-cols-10 gap-2">
                      {Object.entries(gameData.haruf_bahar).map(([number, data]) => (
                        <div key={number} className="p-2 bg-white rounded-lg border border-orange-500/20 text-center">
