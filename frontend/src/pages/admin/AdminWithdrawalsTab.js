...
const AdminWithdrawalsTab = () => {
  ...
  return (
    <Card className="bg-gray-50 border-gray-200">
      <CardHeader><CardTitle className="text-gray-900 font-['Unbounded']">निकासी प्रबंधन</CardTitle></CardHeader>
      <CardContent>
        ...
        {withdrawalSubTab === 'pending' && (
          withdrawals.length === 0 ? (
            <div className="text-center py-8"><p className="text-gray-500">कोई लंबित निकासी नहीं</p></div>
          ) : (
              ...
              {withdrawals.map((w, index) => (
                <div key={index} className="p-4 bg-white rounded-lg border border-gray-100">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <p className="text-gray-900 font-medium">{w.user_name}</p>
                      <p className="text-gray-500 text-sm">{w.user_phone || w.user_email}</p>
                    </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-3 mb-3 space-y-2">
                    {(!w.withdrawal_method || w.withdrawal_method === 'upi') && w.upi_id && (
                      <div className="flex items-center justify-between">
                        <span className="text-gray-500 text-sm">UPI: <span className="text-gray-900">{w.upi_id}</span></span>
                        <button onClick={() => { navigator.clipboard.writeText(w.upi_id); toast.success('UPI ID कॉपी हो गई'); }} className="px-2 py-1 rounded bg-gray-100 text-gray-300 hover:bg-gray-100 text-xs" data-testid={`copy-upi-${w.id}`}>कॉपी</button>
                      </div>
                    )}
                    {w.withdrawal_method === 'bank' && (
                      <>
                        <div className="flex items-center justify-between">
                          <span className="text-gray-500 text-sm">नाम: <span className="text-gray-900">{w.account_holder}</span></span>
                          <button onClick={() => { navigator.clipboard.writeText(w.account_holder || ''); toast.success('कॉपी हो गया'); }} className="px-2 py-1 rounded bg-gray-100 text-gray-300 hover:bg-gray-100 text-xs">कॉपी</button>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-gray-500 text-sm">A/C: <span className="text-gray-900">{w.bank_account}</span></span>
                          <button onClick={() => { navigator.clipboard.writeText(w.bank_account || ''); toast.success('कॉपी हो गया'); }} className="px-2 py-1 rounded bg-gray-100 text-gray-300 hover:bg-gray-100 text-xs">कॉपी</button>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-gray-500 text-sm">IFSC: <span className="text-gray-900">{w.ifsc_code}</span></span>
                          <button onClick={() => { navigator.clipboard.writeText(w.ifsc_code || ''); toast.success('कॉपी हो गया'); }} className="px-2 py-1 rounded bg-gray-100 text-gray-300 hover:bg-gray-100 text-xs">कॉपी</button>
                        </div>
                      </>
                    )}
                    {w.withdrawal_method === 'scanner' && w.scanner_image && (
                      <div>
                        <p className="text-gray-500 text-sm mb-2">स्कैनर:</p>
                        <img src={`${API_URL}${w.scanner_image}`} alt="Scanner" className="w-32 h-32 object-contain rounded-lg border border-gray-200 mb-2" />
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" onClick={() => handleWithdrawalAction(w.id, 'approve')} data-testid={`approve-withdrawal-${w.id}`} className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-gray-900">
        ...
        {withdrawalSubTab === 'approved' && (
          approvedWithdrawals.length === 0 ? (
            <div className="text-center py-8"><p className="text-gray-500">कोई स्वीकृत निकासी नहीं</p></div>
          ) : (
            <div className="space-y-3">
              {approvedWithdrawals.map((w, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-white rounded-lg border border-emerald-500/10">
                  <div>
                    <p className="text-gray-900 font-medium text-sm">{w.user_name}</p>
                    <p className="text-gray-500 text-xs">{w.user_phone || w.user_email} | {w.withdrawal_method === 'bank' ? `बैंक: ${w.bank_account}` : w.withdrawal_method === 'scanner' ? 'स्कैनर' : `UPI: ${w.upi_id}`}</p>
                    <p className="text-gray-500 text-xs">{w.approved_at ? utcDate(w.approved_at).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', hour12: true, hour: '2-digit', minute: '2-digit', timeZoneName: 'short' }) : ''}</p>
                  </div>
        ...
        {withdrawalSubTab === 'rejected' && (
          rejectedWithdrawals.length === 0 ? (
            <div className="text-center py-8"><p className="text-gray-500">कोई अस्वीकृत निकासी नहीं</p></div>
          ) : (
            <div className="space-y-3">
              {rejectedWithdrawals.map((w, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-white rounded-lg border border-red-500/10">
                  <div>
                    <p className="text-gray-900 font-medium text-sm">{w.user_name}</p>
                    <p className="text-gray-500 text-xs">{w.user_phone || w.user_email} | {w.withdrawal_method === 'bank' ? `बैंक: ${w.bank_account}` : w.withdrawal_method === 'scanner' ? 'स्कैनर' : `UPI: ${w.upi_id}`}</p>
                    <p className="text-gray-500 text-xs">{w.rejected_at ? utcDate(w.rejected_at).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', hour12: true, hour: '2-digit', minute: '2-digit', timeZoneName: 'short' }) : ''}</p>
                  </div>
                  <div className="text-right">
